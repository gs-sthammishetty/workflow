...
import com.nirmata.workflow.admin.*;
...
import com.nirmata.workflow.queue.zookeeper.MultiQueueReader;
...
public class WorkflowManagerImpl implements WorkflowManager, WorkflowAdmin {
    ...
    public WorkflowManagerImpl(CuratorFramework curator, QueueFactory queueFactory, String instanceName, List<TaskExecutorSpec> specs, AutoCleanerHolder autoCleanerHolder, Serializer serializer, Executor taskRunnerService, MultiQueueInfo multiQueueInfo) {

        this(curator, queueFactory, instanceName, specs, autoCleanerHolder, serializer, taskRunnerService, true);
        ...
        for (int i = 0; i < multiQueueInfo.getQty(); i++) {
            builder.add(new MultiQueueReader(this.getCurator(), t -> executeTask(multiQueueInfo.getTaskExecutor(), t), this.getSerializer(), multiQueueInfo));
        }
        consumers = builder.build();
    }

    public WorkflowManagerImpl(CuratorFramework curator, QueueFactory queueFactory, String instanceName, List<TaskExecutorSpec> specs, AutoCleanerHolder autoCleanerHolder, Serializer serializer, Executor taskRunnerService, boolean withMultiQueueInfo) {
